...
const ETFS = [{ id: 0, ticker: "SPY", name: "SPDR S&P 500 ETF Trust", region: "US", corr_group: "US Broad Market", expense: "0.09%", aum: "$570B", color: "#3b82f6" }, { id: 1, ticker: "EZU", name: "iShares MSCI Eurozone ETF", region: "Europe", corr_group: "Europe Equity", expense: "0.51%", aum: "$9.2B", color: "#f59e0b" }, { id: 2, ticker: "INDA", name: "iShares MSCI India ETF", region: "India", corr_group: "EM Asia Equity", expense: "0.61%", aum: "$6.1B", color: "#f97316" }, { id: 3, ticker: "KWEB", name: "KraneShares CSI China Internet", region: "China", corr_group: "China Equity", expense: "0.69%", aum: "$5.8B", color: "#ef4444" }, { id: 4, ticker: "EWH", name: "iShares MSCI Hong Kong ETF", region: "Hong Kong", corr_group: "HK Equity", expense: "0.50%", aum: "$0.9B", color: "#ec4899" }, { id: 5, ticker: "GLD", name: "SPDR Gold Shares", region: "Global", corr_group: "Gold / Commodities", expense: "0.40%", aum: "$85B", color: "#fbbf24" }, { id: 6, ticker: "TLT", name: "iShares 20+ Year Treasury Bond", region: "US", corr_group: "Long-Duration Bonds", expense: "0.15%", aum: "$58B", color: "#06b6d4" }, { id: 7, ticker: "DBC", name: "Invesco DB Commodity Index", region: "Global", corr_group: "Multi-Commodity", expense: "0.85%", aum: "$1.8B", color: "#10b981" }, { id: 8, ticker: "VNQ", name: "Vanguard Real Estate ETF", region: "US", corr_group: "US REITs", expense: "0.12%", aum: "$35B", color: "#a78bfa" }, { id: 9, ticker: "EWJ", name: "iShares MSCI Japan ETF", region: "Japan", corr_group: "Japan Equities", expense: "0.50%", aum: "$9.8B", color: "#84cc16" }];
...
const CandlestickTooltip = ({ active, payload, label }) => {
  ...
  return React.createElement(
    "div",
    { style: { background: "#050c18", border: "1px solid #1a3858", borderRadius: "6px", padding: "10px 14px", fontFamily: "'Syne Mono',monospace", fontSize: "11px" } },
    React.createElement(
      "div",
      { style: { color: "#4b6070", marginBottom: "6px" } },
      formatDateCN(label)
    ),
    React.createElement(
      "div",
      { style: { color: "#d1d9e6", marginBottom: "3px" } },
      "O: ",
      React.createElement(
        "span",
        { style: { color: "#8fa8c0" } },
        "$",
        d.open != null ? d.open.toFixed(2) : "-"
      ),
      "  H: ",
      React.createElement(
        "span",
    ...
    React.createElement(
      "div",
      { style: { color: d.dailyReturn >= 0 ? "#22c55e" : "#ef4444" } },
      "Return: ",
      d.dailyReturn >= 0 ? "+" : "",
      d.dailyReturn != null ? d.dailyReturn.toFixed(2) : "-",
      "%"
    )
  );
};
...
export default function App() {
      ...
      try {
        ...
        Object.keys(raw || {}).forEach(ticker => {
          const rows = ((raw[ticker] || {}).records || []).map((r, idx, arr) => {
            ...
            return _extends({}, r, { dailyReturn: ret });
          });
  ...
  useEffect(() => {
    const loadSymbolLookup = async () => {
      try {
        ...
        const cleaned = rows.filter(r => r && r.symbol).map(r => ({
  ...
  useEffect(() => {
    ...
    let cutoff = new Date();
    if (timeRange === "1M") cutoff.setMonth(today.getMonth() - 1);else if (timeRange === "3M") cutoff.setMonth(today.getMonth() - 3);else if (timeRange === "1Y") cutoff.setFullYear(today.getFullYear() - 1);else cutoff = new Date("2019-01-01");
  ...
  const cacheFetchedSeries = (ticker, records) => {
    ...
    setMarketData(prev => _extends({}, prev, { [ticker]: records }));
    try {
  ...
  const addSymbolToInput = symbol => {
    const current = queryInput.split(",").map(s => s.trim().toUpperCase()).filter(Boolean);
  ...
  return React.createElement(
    ...
    React.createElement("link", { href: "https://fonts.googleapis.com/css2?family=Syne:wght@400;500;600;700;800&family=Syne+Mono&display=swap", rel: "stylesheet" }),
    React.createElement(
      "style",
      null,
      `
    ...
    React.createElement(
      ...
      ETFS.map(e => React.createElement(
        "button",
        { key: e.ticker, className: `etf-pill ${selected.ticker === e.ticker ? "on" : ""}`, style: { "--c": e.color }, onClick: () => setSelected(e) },
        e.ticker,
        " ",
        React.createElement(
          "span",
          { style: { opacity: .5, fontSize: "9px" } },
          e.region
        )
      ))
    ),
    (dataLoading || dataError) && React.createElement(
      "div",
      { style: { padding: "10px 28px", borderBottom: "1px solid #08172a", fontFamily: "'Syne Mono',monospace", fontSize: "11px", color: dataError ? "#ef4444" : "#8fa8c0" } },
      dataError ? `Nasdaq data load failed: ${dataError}` : "Loading Nasdaq daily OHLC data..."
    ),
    React.createElement(
      "div",
      { style: { padding: "24px 28px" }, className: "fade", key: tab + selected.ticker },
      tab === "matrix" && React.createElement(
        "div",
        null,
        React.createElement(
          "div",
          ...
          React.createElement(
            ...
            [["AUM", selected.aum], ["Expense", selected.expense], ["Avg Corr", avgCorr]].map(([l, v]) => React.createElement(
              "div",
              { key: l, style: { background: "#060e1c", border: "1px solid #0a1e32", borderRadius: "8px", padding: "14px 18px", minWidth: "100px" } },
              React.createElement(
                "div",
        ...
        React.createElement(
          ...
          React.createElement(
            "div",
            { style: { marginBottom: "20px", padding: "16px", background: "#030810", border: "1px solid #0a1e32", borderRadius: "8px" } },
            React.createElement(
              "div",
            ...
            React.createElement(
              ...
              ETFS.map(e => {
                ...
                return React.createElement(
                  "button",
                  { key: e.id, className: `filter-pill ${isVisible ? "on" : ""}`, style: { "--c": e.color }, onClick: () => {
                      if (isVisible && visibleETFs.length <= 2) return;setVisibleETFs(isVisible ? visibleETFs.filter(id => id !== e.id) : [...visibleETFs, e.id].sort((a, b) => a - b));
                    } },
                  isVisible ? "✓ " : "+ ",
                  e.ticker
                );
              ...
              React.createElement(
                "button",
                { className: "filter-pill", onClick: () => setVisibleETFs(ETFS.map(e => e.id)), style: { marginLeft: "8px" } },
                "Select All"
              )
            )
          ),
          React.createElement(
            "div",
            { style: { fontFamily: "'Syne Mono',monospace", fontSize: "10px", color: "#1e3a55", letterSpacing: ".1em", marginBottom: "20px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "10px" } },
            React.createElement(
              "span",
              null,
            ...
            React.createElement(
              "div",
              { style: { display: "flex", gap: "10px", fontSize: "9px", flexWrap: "wrap" } },
              [["≥0.7", "#dc2626"], ["0.5–0.7", "#f97316"], ["0.3–0.5", "#eab308"], ["0.1–0.3", "#84cc16"], ["<0.1", "#22c55e"]].map(([r, c]) => React.createElement(
                "span",
                { key: r, style: { display: "flex", alignItems: "center", gap: "4px" } },
                React.createElement("span", { style: { width: "8px", height: "8px", borderRadius: "2px", background: c, display: "inline-block" } }),
                React.createElement(
                  "span",
                  { style: { color: "#2d4a65" } },
                  r
          ...
          React.createElement(
            ...
            displayETFs.map(e => React.createElement(
              "div",
              { key: e.ticker, style: { textAlign: "center", fontFamily: "'Syne Mono',monospace", fontSize: "10px", color: e.ticker === selected.ticker ? e.color : "#2d4a65", padding: "4px 2px", fontWeight: e.ticker === selected.ticker ? 700 : 400 } },
              e.ticker
            )),
            displayETFs.map(row => React.createElement(
              React.Fragment,
              { key: row.ticker + "_row" },
              React.createElement(
                "div",
              ...
              displayETFs.map(col => {
                const ri = row.id,
                      ...
                      v = corrMatrix[ri][ci];
                const isSelf = ri === ci,
                      isHighlight = ri === selected.id || ci === selected.id;
                return React.createElement(
                  "div",
                  { key: col.ticker, className: "corr-cell",
                    ...
                    onClick: () => {
                      if (!isSelf) setSelectedPair({ a: row, b: col, corr: v });
                    } },
                  isSelf ? "—" : v.toFixed(3)
                );
              })
          ...
          selectedPair && React.createElement(
            "div",
            { style: { marginTop: "24px", padding: "20px", background: "#0a1e32", border: "1px solid #1a3858", borderRadius: "8px" }, className: "fade" },
            React.createElement(
              "div",
              ...
              React.createElement(
                ...
                React.createElement(
                  ...
                  selectedPair.a.ticker
                ),
                " \xD7 ",
                React.createElement(
                  "strong",
                  { style: { color: selectedPair.b.color } },
                  selectedPair.b.ticker
                )
              ),
              React.createElement(
                "button",
                { onClick: () => setSelectedPair(null), style: { background: "transparent", border: "none", color: "#4a6a85", cursor: "pointer", fontFamily: "'Syne Mono',monospace" } },
                "\u2715 CLOSE"
              )
            ),
            React.createElement(
          ...
          highlight && highlight.ri !== highlight.ci && React.createElement(
            "div",
      ...
      tab === "custom" && React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            "Enter 2-5 tickers or company names (example: ",
            React.createElement(
              "span",
              { style: { color: "#8fb8d8" } },
              "AAPL, Microsoft, NVIDIA"
            ),
          ...
          React.createElement(
            ...
            React.createElement("input", {
              value: queryInput,
              onChange: e => {
                setQueryInput(e.target.value);setQueryResolved([]);
              },
          ...
          React.createElement(
            "div",
            { style: { marginTop: "10px", background: "#030810", border: "1px solid #0a1e32", borderRadius: "8px", padding: "10px 12px" } },
            React.createElement(
              "div",
          ...
          queryResolved.length > 0 && React.createElement(
            "div",
            { style: { marginTop: "8px", fontFamily: "'Syne Mono',monospace", fontSize: "10px", color: "#8fa8c0", display: "flex", gap: "8px", flexWrap: "wrap" } },
            React.createElement(
              "span",
              { style: { color: "#1e3a55" } },
              "RESOLVED INPUT:"
            ),
            queryResolved.map(r => React.createElement(
              "span",
              { key: `${r.input}-${r.symbol}`, style: { background: "#030810", border: "1px solid #0a1e32", borderRadius: "4px", padding: "3px 6px" } },
              r.input,
              " \u2192 ",
              React.createElement(
                "span",
      ...
      tab === "chart" && React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              "div",
              { style: { fontSize: "12px", color: "#2d4a65", marginTop: "4px", fontFamily: "'Syne Mono',monospace" } },
              "Daily OHLC K-Line \xB7 ",
              chartData.length,
              " trading days"
            )
          ),
          React.createElement(
            ...
            React.createElement(
              ...
              timeRange === "ALL" ? "Full Period" : timeRange,
              " Return"
            )
          )
        ),
        React.createElement(
          ...
          React.createElement(
            "span",
            { style: { fontFamily: "'Syne Mono',monospace", fontSize: "10px", color: "#1e3a55", marginRight: "4px" } },
            "TIME RANGE:"
          ),
          ["1M", "3M", "1Y", "ALL"].map(r => React.createElement(
            "button",
            { key: r, className: `range-btn ${timeRange === r ? "on" : ""}`, onClick: () => setTimeRange(r) },
            r
          )),
          React.createElement(
            "span",
            { style: { marginLeft: "16px", fontFamily: "'Syne Mono',monospace", fontSize: "10px", color: "#1e3a55" } },
            chartData[0] && chartData[0].date ? formatDateCN(chartData[0].date) : "—",
            " \u2192 ",
            chartData[chartData.length - 1] && chartData[chartData.length - 1].date ? formatDateCN(chartData[chartData.length - 1].date) : "—"
          )
        ),
        React.createElement(
          "div",
          ...
          React.createElement(
            ...
            React.createElement(
              ComposedChart,
              { data: chartData, margin: { top: 5, right: 10, bottom: 5, left: 0 } },
              React.createElement(
                "defs",
                null,
                React.createElement(
                  "linearGradient",
                  { id: "ag", x1: "0", y1: "0", x2: "0", y2: "1" },
                  React.createElement("stop", { offset: "5%", stopColor: selected.color, stopOpacity: 0.2 }),
                  React.createElement("stop", { offset: "95%", stopColor: selected.color, stopOpacity: 0 })
                )
              ),
              React.createElement(CartesianGrid, { strokeDasharray: "2 5", stroke: "#0a1a2e" }),
              React.createElement(XAxis, { dataKey: "date", stroke: "#0a1a2e", tick: { fill: "#1e3a55", fontSize: 9, fontFamily: "Syne Mono" },
                tickFormatter: v => {
                  const [y, m, d] = v.split("-");
              ...
              React.createElement(YAxis, { stroke: "#0a1a2e", tick: { fill: "#1e3a55", fontSize: 9, fontFamily: "Syne Mono" }, tickFormatter: v => `$${v}`, width: 55, domain: ['auto', 'auto'] }),
              React.createElement(Tooltip, { content: React.createElement(CandlestickTooltip, null) }),
              React.createElement(Line, { type: "monotone", dataKey: "close", stroke: selected.color, strokeWidth: 1.2, dot: false }),
              React.createElement(Scatter, {
                ...
                shape: props => {
                  ...
                  return React.createElement("rect", { x: cx - 1.5, y: cy - 1.5, width: 3, height: 3, fill: color });
                }
              })
            )
          ),
          React.createElement(
            "div",
            { style: { display: "flex", justifyContent: "space-between", marginTop: "12px", fontFamily: "'Syne Mono',monospace", fontSize: "10px", color: "#1e3a55" } },
            React.createElement(
              "span",
              null,
              "Open: ",
              React.createElement(
                ...
                { style: { color: "#8fa8c0" } },
                "$",
                chartData[0] && chartData[0].open != null ? chartData[0].open.toFixed(2) : "-"
              )
            ),
            React.createElement(
              "span",
              ...
              React.createElement(
                ...
                { style: { color: "#22c55e" } },
                "$",
                Number.isFinite(Math.max(...chartData.map(d => d.high || 0))) ? Math.max(...chartData.map(d => d.high || 0)).toFixed(2) : "-"
              )
            ),
            React.createElement(
              "span",
              ...
              React.createElement(
                ...
                { style: { color: "#ef4444" } },
                "$",
                Number.isFinite(Math.min(...chartData.map(d => d.low || Infinity))) ? Math.min(...chartData.map(d => d.low || Infinity)).toFixed(2) : "-"
              )
            ),
            React.createElement(
              "span",
              ...
              React.createElement(
                ...
                { style: { color: "#d1d9e6" } },
                "$",
                chartData[chartData.length - 1] && chartData[chartData.length - 1].close != null ? chartData[chartData.length - 1].close.toFixed(2) : "-"
              )
            )
          )
        ),
        React.createElement(
          "div",
          { style: { background: "#060e1c", border: "1px solid #0a1e32", borderRadius: "10px", padding: "20px" } },
          React.createElement(
            "div",
          ...
          React.createElement(
            ...
            ETFS.filter(e => e.ticker !== selected.ticker).map(e => {
              ...
              return React.createElement(
                "div",
                { key: e.ticker, style: { background: corrBg(v), border: `1px solid ${corrColor(v)}33`, borderRadius: "6px", padding: "10px 14px", minWidth: "90px" } },
                React.createElement(
                  "div",
      ...
      tab === "report" && React.createElement(
        "div",
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            "ETF Selection Rationale \u2014 Correlation Analysis Study"
          ),
          React.createElement(
            "div",
          ...
          React.createElement(
            ...
            "The 10 ETFs in this study were selected to ",
            React.createElement(
              "strong",
              { style: { color: "#d1d9e6" } },
              "maximize structural independence of return drivers"
            ),
            ". Each product is governed by materially different macroeconomic, policy, and fundamental forces."
          ),
          React.createElement(
            "div",
            ...
            React.createElement(
              ...
              React.createElement(
                "strong",
                { style: { color: "#d1d9e6" } },
                "asset class independence"
              ),
              " (equities, fixed income, real assets), ",
              React.createElement(
                "strong",
                { style: { color: "#d1d9e6" } },
                "sector/industry independence"
              ),
              ", and ",
              React.createElement(
                "strong",
                { style: { color: "#d1d9e6" } },
                "crisis behavior independence"
              ),
              "."
            )
          ),
          React.createElement(
            ...
            React.createElement(
              "div",
              { style: { fontFamily: "'Syne Mono',monospace", fontSize: "11px", color: "#22c55e", letterSpacing: ".08em", marginBottom: "16px" } },
              "02 \u2014 DAILY RETURN CORRELATION SUMMARY"
            ),
            React.createElement(
              "div",
              { style: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" } },
              [["SPY vs EZU", "0.815 — Strong linkage via Western economies, shared banking systems"], ["SPY vs GLD", "0.111 — Near-zero, driven by real interest rates vs earnings"], ["SPY vs TLT", "-0.136 — Negative, classic flight-to-safety 60/40 relationship"], ["GLD vs TLT", "0.229 — Low, both safe-havens but different inflation responses"], ["TLT vs DBC", "-0.160 — Negative, inflation trade: commodities up, bonds down"], ["KWEB vs TLT", "-0.031 — Near zero, Chinese tech disconnected from US rates"], ["INDA vs KWEB", "0.343 — Low, India benefits from China+1 supply chain shift"], ["EWJ vs KWEB", "0.412 — Low, BOJ vs PBOC policy environments are divergent"]].map(([pair, desc]) => React.createElement(
                "div",
                { key: pair, style: { background: "#070f1e", border: "1px solid #0a1a2e", borderRadius: "6px", padding: "12px 14px" } },
                React.createElement(
                  "div",
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              " (SPY/TLT, TLT/DBC) providing genuine portfolio diversification \u2014 not mere sector rotation."
            )
          ),
          React.createElement(
            "div",
      ...
      tab === "sources" && React.createElement(
        "div",
        { style: { maxWidth: "860px", margin: "0 auto" }, className: "fade" },
        React.createElement(
          "div",
          { style: { background: "#060e1c", border: "1px solid #0a1e32", borderRadius: "12px", padding: "36px 40px" } },
          React.createElement(
          ...
          React.createElement(
            ...
            "Data Sources and Computation Methodology"
          ),
          React.createElement(
            "div",
            { style: { marginBottom: "28px" } },
            React.createElement(
              ...
              "DATA SOURCE \u2014 NASDAQ (NASDAQ.COM)"
            ),
            React.createElement(
              "div",
              { style: { fontSize: "13px", color: "#7a9ab5", lineHeight: 1.8, background: "#030810", padding: "16px", borderRadius: "6px", border: "1px solid #0a1a2e" } },
              "All daily OHLC (Open, High, Low, Close) price data is fetched directly from ",
              React.createElement(
                "strong",
                { style: { color: "#d1d9e6" } },
                "Nasdaq.com"
              ),
              " historical API for each ETF. Data spans ",
              React.createElement(
                "strong",
                { style: { color: "#d1d9e6" } },
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                "code",
                { style: { color: "#8fb8d8" } },
                "return_t = (Close_t - Close_t-1) / Close_t-1 \xD7 100"
              ),
              ". Pearson coefficients are recalculated for all ETF pairs using shared trading dates."
            )
          ),
          React.createElement(
            "div",
            ...
            React.createElement(
              "div",
              { style: { fontFamily: "'Syne Mono',monospace", fontSize: "11px", color: "#22c55e", letterSpacing: ".08em", marginBottom: "12px" } },
              "VERIFIED ANNUAL RETURNS"
            ),
            React.createElement(
              "div",
              { style: { display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: "8px" } },
              [["SPY", "+184.28%"], ["EZU", "+90.80%"], ["INDA", "+51.73%"], ["KWEB", "-22.12%"], ["EWH", "+5.24%"], ["GLD", "+258.74%"], ["TLT", "-28.99%"], ["DBC", "+103.10%"], ["VNQ", "+29.57%"], ["EWJ", "+71.94%"]].map(([ticker, ret]) => React.createElement(
                "div",
                { key: ticker, style: { background: "#030810", border: "1px solid #0a1a2e", borderRadius: "4px", padding: "10px 12px", display: "flex", justifyContent: "space-between" } },
                React.createElement(
                  "span",
                  { style: { fontFamily: "'Syne Mono',monospace", fontSize: "11px", color: "#8fb8d8" } },
                  ticker
                ),
                React.createElement(
                  "span",
                  { style: { fontFamily: "'Syne Mono',monospace", fontSize: "11px", color: ret.startsWith("+") ? "#22c55e" : "#ef4444" } },
                  ret
                )
              ))
            )
          ...
          React.createElement(
            ...
            " Data sourced from Nasdaq.com. The model displays exact trading-day OHLC rows. This tool is for research use only and does not constitute investment advice."
          )
        )
      )
    )
