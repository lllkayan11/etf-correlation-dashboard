...
export default function App() {
  ...
  const totalDirectoryPages = useMemo(
    () => Math.max(1, Math.ceil(allFilteredSymbolDirectory.length / DIRECTORY_PAGE_SIZE)),
    [allFilteredSymbolDirectory.length]
  );
    ...
    try {
      ...
      const results = await Promise.all(unique.map(async (t) => {
        ...
        try {
          ...
          if (!proxyBaseUrl) {
            throw new Error(
              `Cannot load ${t} from Nasdaq in browser-only mode. Please configure proxy URL in proxy-config.json, or pick symbols from stored directory.`
            );
          }
            ...
            <div style={{background:"#060e1c",border:"1px solid #0a1e32",borderRadius:"12px",padding:"24px"}}>
              ...
              <div style={{marginTop:"10px",background:"#030810",border:"1px solid #0a1e32",borderRadius:"8px",padding:"10px 12px"}}>
                ...
                <div style={{display:"flex",gap:"8px",marginBottom:"8px",alignItems:"center",flexWrap:"wrap"}}>
                  <button className={`range-btn ${directoryMode==="stored"?"on":""}`} onClick={()=>setDirectoryMode("stored")}>Stored Only</button>
                  <button className={`range-btn ${directoryMode==="all"?"on":""}`} onClick={()=>setDirectoryMode("all")}>All Lookup</button>
                  <span style={{fontFamily:"'Syne Mono',monospace",fontSize:"10px",color:"#4a6a85"}}>
                    Stored: {storedSymbolSet.size} symbols
                  </span>
                  <span style={{fontFamily:"'Syne Mono',monospace",fontSize:"10px",color:"#4a6a85"}}>
                    Page {symbolPage}/{totalDirectoryPages}
                  </span>
                  <button className="range-btn" onClick={()=>setSymbolPage((p)=>Math.max(1,p-1))} disabled={symbolPage<=1}>Prev</button>
                  <button className="range-btn" onClick={()=>setSymbolPage((p)=>Math.min(totalDirectoryPages,p+1))} disabled={symbolPage>=totalDirectoryPages}>Next</button>
                </div>
                <input
                  value={symbolSearch}
